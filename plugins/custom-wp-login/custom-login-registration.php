 ...
 * Description: Adds custom form/pages for login and register via shortcode.
 * Version: 1.0
 * Author: Alik Welyn
...
class Custom_User_Flow {
	...
	public function register_settings_fields() {
		//reCaptcha keys
		register_setting( 'general', 'enable-recaptcha' );
		...
		add_settings_field(
			'enable-recaptcha',
			'<label for="enable-recaptcha">' . __( 'enable reCAPTCHA', 'custom-wp-login' ) . '</label>',
			array( $this, 'enable_recaptcha' ),
			'general'
		);
		
		add_settings_field(
			'personalise-login-recaptcha-site-key',
			'<label for="personalise-login-recaptcha-site-key">' . __( 'reCAPTCHA site key', 'custom-wp-login' ) . '</label>',
		...
		add_settings_field(
			...
			'<label for="personalise-login-recaptcha-secret-key">' . __( 'reCAPTCHA secret key', 'custom-wp-login' ) . '</label>',
	...
	public function enable_recaptcha() {
		$value = get_option( 'enable-recaptcha', '' );
		echo '
		<select id="enable-recaptcha" class="regular-text" name="enable-recaptcha" />
		<option value="' . esc_attr( $value ) . '">Yes</option>
		<option value="' . esc_attr( $value ) . '">No</option>
		</select>
		';
	}
	...
	public static function plugin_activated() {
		...
		$page_definitions = array(
			...
			'register' => array(
				'title' => __( 'Register', 'custom-wp-login' ),
			...
			'password-reset' => array(
				'title' => __( 'Password Reset', 'custom-wp-login' ),
	...
	public function render_register_form( $attributes, $content = null ) {
		...
		if( is_user_logged_in() ) {
			...
			return __( 'Registering is not allowed.', 'custom-wp-login' );
		} else {
	...
	public function render_account_info_page( $attributes, $content = null ) {
		...
		if( is_user_logged_in() ) {
			...
			return __( 'Please log in.', 'custom-wp-login' );
			exit;
		}
	}
	...
	private function get_error_message( $error_code ) {
		switch( $error_code ) {
			case 'empty_username':
				return __( 'The username field is empty', 'custom-wp-login' );
			
			case 'empty_password':
				return __( 'The password field is empty', 'custom-wp-login' );
			
			case 'invalid_username':
				return __( 'Incorrect login details, please try again.<br><a href="%s">Forgot your password?</a>', 'custom-wp-login' );
			
			case 'incorrect_password':
				$err = __('Incorrect login details, please try again.<br><a href="%s">Forgot your password?</a>', 'custom-wp-login');
				return sprintf( $err, wp_lostpassword_Url() );
				
			case 'email':
				return __( 'Email you entered is not valid', 'custom-wp-login' );
				
			case 'user_exists':
				return __( 'User already registered', 'custom-wp-login');
				
			case 'email_exists':
				return __( 'Email already registered', 'custom-wp-login');
				
			case 'closed':
				return __( 'Registration is closed', 'custom-wp-login');
				
			case 'captcha':
				return __( 'reCaptcha failed', 'custom-wp-login' );
				
			case 'password_mismatch':
				return __( "Passwords don't match", 'custom-wp-login' );
				
			case 'password_tooshort':
				return __( "Password needs to be at least 7 characters long", 'custom-wp-login' );
				
			case 'empty_username':
				return __( 'Enter username to continue', 'custom-wp-login' );
				
			case 'invalid_email':
			case 'invalidcombo':
				return __( 'No users with that email', 'custom-wp-login' );
				
			case 'expiredkey':
			case 'invalidkey':
				return __( 'The password reset link is not valid anymore', 'custom-wp-login' );
				
			case 'password_reset_mismatch':
				return __( 'The passwords do not match', 'custom-wp-login' );
			
			case 'password_reset_empty':
				return __( 'Cannot accept empty passwords', 'custom-wp-login' );
				
			default:
				break;
		}
		
		return __( 'Unknown error', 'custom-wp-login' );
	}
	...
	private function register_user( $username, $password, $first_name, $last_name, $email, $zip, $address, $number, $district, $city, $state ) {
		$errors = new WP_Error();
		...
		add_user_meta( $user_id, 'number', $_POST['number'] ); 
	...
	public function do_register_user() {
		if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
			...
			if( !get_option( 'users_can_register' ) ) {
				...
				$result = $this->register_user( $username, $password, $first_name, $last_name, $email, $zip, $address, $number, $district, $city, $state );
				...
				if( is_wp_error( $result ) ) {
					...
					// Success
					//$redirect_url = add_query_arg( 'registered', $email, home_url( 'account' ) );
					$redirect_url = add_query_arg( 'registered', home_url( 'account' ) );
				}
			}
	...
	public function do_password_reset() {
		if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
			...
			if( isset( $_POST['pass1'] ) ) {
				...
				_e( 'Invalid request', 'custom-wp-login' );
			}
	...
	public function replace_retrieve_password_message( $message, $key, $user_login, $user_data ) {
		$msg = __( 'Hello!', 'custom-wp-login' ) . "<br><br>";
		$msg .= sprintf( __( 'You asked to reset your password for the account: %s', 'custom-wp-login' ), $user_login ) . "<br><br>";
		$msg .= __( "If this is a mistake, ignore this email.", 'custom-wp-login' ) . "<br><br>";
		$msg .= __( 'To reset your password, follow this link:', 'custom-wp-login' ) . "<br><br>";
		$msg .= esc_url( site_url( "wp-login.php?action=rp&key=".$key."&login=".$user_login, 'login' ) ) . "<br><br>";
		$msg .= sprintf( __( '%s', 'custom-wp-login' ), get_bloginfo( 'name' ) );
